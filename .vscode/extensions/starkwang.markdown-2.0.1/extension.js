...
const { smms } = require('./oss/smms')
const fs = require('fs')
const {spawn} = require('child_process')
const qnUpload = require('./lib/upload')
exports.activate = (context) => {
  const disposable = vscode.commands.registerCommand('extension.figimg', () => {
    start()
  })
  ...
  createImageDirWithImagePath(imagePath).then(imagePath => {
    saveClipboardImageToFileAndGetPath(imagePath, (imagePath) => {
      ...
      if (imagePath === 'no image') {
        vscode.window.showErrorMessage('There is not a image in clipboard.', 3000)
        return
      }
      console.log('imagePath', imagePath)
      smms(imagePath).then(item=>{
        console.log('item',item)
        if(item.status){
          const img = `![${item.storename}](${item.url})`
          report('upload success', { data: {img,url:item.url}, err: false }, 'error')
